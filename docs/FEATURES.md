...
**Status:** done · **Phase:** 8
**Spec:** Replace in-memory keyword search in `ChatIndexing` with an `FTSIndex` interface backed by `InMemoryFTS` (tests) and `PostgresFTS` (production). PostgresFTS uses `to_tsvector`/`plainto_tsquery` on a `fts_documents` table with a GIN index.
**Acceptance:** `InMemoryFTS` passes all 9 FTS search tests including case-insensitive prefix matching, limit, delete, and clear. `PostgresFTS` SQL is verified by code review (no live-DB required in unit tests).
**Test:** `packages/memory/test/fts-search.spec.ts`

### F-203 — Procedural memory (skill linkage)

**Status:** done · **Phase:** 8
**Spec:** Skills generated by `SkillReflector` can be indexed into the FTS layer with `type: skill` metadata. Searching with `filterMetadata: { type: 'skill' }` returns only skill documents, not chat messages.
