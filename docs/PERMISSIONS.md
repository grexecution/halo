...
Source of truth at `~/.open-greg/permissions.yml`. Hot-reloaded on change (chokidar).
...
**Headless-Linux fallback** (no keyring daemon): AES-256-GCM encrypted file at `~/.open-greg/secrets.enc`. Passphrase from env var `GREG_SECRET_PASSPHRASE` (required at service start — fails fast if missing). Wizard generates a random passphrase on first run and prints it once for the user to store.
...
- Commit `~/.open-greg/secrets.enc` (it's under the user home, not the repo, but belt-and-braces).
