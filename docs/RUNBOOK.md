...
**Fix:** set `CLAW_SECRET_PASSPHRASE` to a strong random string in your shell before re-running the wizard. The wizard detects keytar failure and offers this path automatically, but if you skipped the prompt:
```bash
export CLAW_SECRET_PASSPHRASE="$(openssl rand -base64 32)"
echo "export CLAW_SECRET_PASSPHRASE='$CLAW_SECRET_PASSPHRASE'" >> ~/.bashrc
...
- Didn't pick an LLM in the wizard. Re-run `npx create-claw-alt init` and pick one.
...
npx create-claw-alt init --skip-optional
```
...
```bash
docker logs -f claw-alt-browser
```
Most common causes:
...
- User's persistent profile got corrupted — `rm -rf ~/.claw-alt/browser-profile` and re-login.
...
- Permanent: edit `~/.claw-alt/permissions.yml` → `budgets` section.

### "Missing permission: tools.shell_sudo" on a command that doesn't need sudo
The shell parser matches `sudo` at start of command. If you have an alias, it's evaluated *before* the parser. Best not to use aliased sudo in tool commands. Use `/usr/bin/sudo` explicitly if needed.
...
**Fix:** run `pnpm tunnel` which starts a Cloudflare Tunnel pointing at the control plane and registers the new URL with the connector provider automatically. Persist the tunnel URL in `~/.claw-alt/config.yml`.

### Dashboard shows "Control plane unreachable"
1. `docker ps | grep control-plane` — is it running?
2. `docker logs claw-alt-control-plane` — errors?
...
3. Generate a new `CLAW_SECRET_PASSPHRASE`.
...
### From v0.x to v0.(x+1)
```bash
npx create-claw-alt upgrade
```
This:
1. Backs up DB to `~/.claw-alt/backups/pre-upgrade-YYYYMMDD.sql.gz`.
...
```bash
pnpm db:restore ~/.claw-alt/backups/pre-upgrade-YYYYMMDD.sql.gz
```
...
```bash
docker exec claw-alt-qdrant curl localhost:6333/collections/openmemory
```
...
rm -rf ~/.claw-alt                   # drops keychain fallback + profiles
# keytar entries remain; remove manually if needed
npx create-claw-alt init
```
This is a last resort. Back up `~/.claw-alt/backups/` first if you want to keep memories.
